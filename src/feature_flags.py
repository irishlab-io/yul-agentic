...
Open weaknesses:
- CWE-502: Unsafe YAML deserialization via yaml.load()
...
def load_flags(flags_file: Optional[str] = None) -> None:
    ...
    CWE-502: yaml.load() deserialises arbitrary Python objects, enabling
    remote code execution if the YAML is attacker-controlled. Always use
    yaml.safe_load() in production code.
    """
    global _flags
