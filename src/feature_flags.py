...
Security fix applied (issue #29 – CVE-2020-14343):
- CWE-502: Replaced yaml.load(Loader=yaml.Loader) with yaml.safe_load()
            to prevent arbitrary Python-object deserialisation / RCE.
"""
    ...
    if os.path.exists(flags_file):
        try:
            with open(flags_file, "r") as fh:
                # FIX (CVE-2020-14343 / CWE-502): Use yaml.safe_load() instead
                # of yaml.load(Loader=yaml.Loader) to prevent arbitrary Python
                # object deserialisation and remote code execution.
