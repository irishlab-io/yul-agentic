...
Security notes (remaining intentional vulnerabilities for educational use):
...
Remediated:
- CWE-502: Replaced yaml.load() (full Loader) with yaml.safe_load() to
            prevent arbitrary Python object instantiation / code execution.
            Library also upgraded from pyyaml 5.3.1 to 6.0.2 to address
            CVE-2020-14343 (incomplete fix for CVE-2020-1747).
"""
    ...
    if os.path.exists(flags_file):
        try:
            with open(flags_file, "r") as fh:
                # Remediation for CWE-502 / CVE-2020-14343: use yaml.safe_load()
                # instead of yaml.load() with the full Loader to prevent
                # arbitrary Python object instantiation from attacker-controlled YAML.
