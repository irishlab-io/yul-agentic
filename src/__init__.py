...
def create_app():
    ...
    def log_feature_flag_override():
        ...
        if override:
            ...
            print(
                f"[FeatureFlags] Override requested: {override} from {request.remote_addr}"
            )
    ...
    def login():
        ...
        if result["success"]:
            ...
            response.set_cookie(
                "session_token", result["session_token"], httponly=False
            )
            return response
        else:
    ...
    def todo_detail(todo_id):
        ...
        return render_template(
            "todo_detail.html", todo=todo, files=files, username=auth_info["username"]
        )
    ...
    def create_todo():
        ...
        result = database.create_todo(
            auth_info["user_id"], title, description, priority
        )
    ...
    def admin_page():
        ...
        return render_template(
            "admin.html", users=[dict(u) for u in users], username=auth_info["username"]
        )
